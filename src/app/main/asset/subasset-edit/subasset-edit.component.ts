...
@Component({
  selector: 'app-subasset-edit',
  templateUrl: './subasset-edit.component.html',
  styleUrls: ['./subasset-edit.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class SubassetEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap
      .subscribe((params: any) => {
        this._http.get(Url + 'api/asset/GetSubAssetByBarCodeId/' + params.params.id).subscribe((data: any) => {
          if (data.status) {
  ...
  Save() {
    ...
    var locationUrl = Url + "api/Asset/UpdateSubAsset";
    ...
    this.modelForm.subAssets.barCodeID = localStorage.getItem('asset_barcodeId');

    if (this.modelForm.subAssets.datePurchased == null) this.modelForm.subAssets.datePurchased = null;
    else if (this.modelForm.subAssets.datePurchased.length == 1)
      this.modelForm.subAssets.datePurchased = this.modelForm.subAssets.datePurchased[0];

    if (this.modelForm.subAssets.warrantyExpiryDate == null) this.modelForm.subAssets.warrantyExpiryDate = null;
    else if (this.modelForm.subAssets.warrantyExpiryDate.length == 1) this.modelForm.subAssets.warrantyExpiryDate = this.modelForm.subAssets.warrantyExpiryDate[0];

    if (this.modelForm.subAssets.nextMaintenanceDate == null) this.modelForm.subAssets.nextMaintenanceDate = null;
    else if (this.modelForm.subAssets.nextMaintenanceDate.length == 1) this.modelForm.subAssets.nextMaintenanceDate = this.modelForm.subAssets.nextMaintenanceDate[0];

    if (this.modelForm.subAssets.endOfLifeDate == null) this.modelForm.subAssets.endOfLifeDate = null;
    else if (this.modelForm.subAssets.endOfLifeDate.length == 1) this.modelForm.subAssets.endOfLifeDate = this.modelForm.subAssets.endOfLifeDate[0];
    this._http.post(locationUrl, this.modelForm.subAssets).subscribe((data: any) => {
      ...
      this._http.post(UploadAssetImages, objFormData).subscribe(() => {
        ...
        this.commonService.simpleAlertMethod('Asset has been updated', 'asset/view/' + this.modelForm.id);
        this.onSetAssetValue(this.modelForm);
      });
