...
import { AssetConditionDatatablesService } from './datatables.service';
...
export class AssetConditionComponent implements OnInit {
   ...
   */
  constructor(private _http: HttpClient, private _datatablesService: AssetConditionDatatablesService, private modalService: NgbModal, private _coreTranslationService: CoreTranslationService, private _assetConditionService: AssetConditionService) {
  ...
  modalOpenLG(modalLG, Id) {
    this._http.get(Url + "api/References/Condition/" + Id).subscribe((data: any) => {
      if (data.status) {
  ...
  exportCSVFile() {
    this._assetConditionService.GetExportCSVFileData().subscribe((data: any) => {
      if (data.status) {
